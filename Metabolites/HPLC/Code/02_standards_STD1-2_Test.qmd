...
The chromatograms are extracted from the .lcd files using chromConverter. These HPLC files include data from 3 channels: RID, UV 210 nm, and UV 260 nm.
...
table.out = "/home/tolonen/Github/actolonen/Public/Analysis_Lab/Metabolites/HPLC/Data/standards_regressions_STD12.tsv";
...
# plot peaks
 plot.peaks = ggplot(peaks.all, aes(x=Minutes, y=Intensity, group=Sample, color=Sample)) +
...
Peaks for all compounds can be discriminated and quantified.

1.  Ethanol peak is slightly overlapping with acetone, but quantification looks OK.
2.  Glucose peaks are problematic, but this is likely experimental error and not an HPLC problem.
