...
files.chrom.df = files.chrom.df %>%
  ...
  filter(!grepl("blanc", files.chrom));
...
Based on a previous run of the code, I am selecting revising the input from the linear models to best quantify the compounds.

```{r modify LM data, eval=F}

regressions.in = regressions.in %>%
  mutate(Peak.start = if_else((Compound == "Formate" & Detector == "SPD-20A 210nm"), true=16.4, false = Peak.start)) %>%
  mutate(Peak.start = if_else((Compound == "Formate" & Detector == "RID-10A"), true=16.6, false = Peak.start));
...
for (my.sample in sample.list) # for each sample
{
  ...
  for(counter in 1:nrow(regressions.in)) 
  {
    ...
    peak.start = row$Peak.start;
    peak.stop = row$Peak.end;
    my.slope = row$Slope;
    my.intercept = row$Y.Intercept;
...
-   [Here](https://drive.google.com/drive/u/0/folders/1nQtbVMlfAlNJVuUk17FjbCjntyMRQwlL) is are plots of the peaks for each compound in each sample.
