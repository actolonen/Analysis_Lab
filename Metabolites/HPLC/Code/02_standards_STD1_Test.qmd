...
table.standards = kable(standards, caption = "Table 1: Metabolite concentrations in STD1 standards");
table.standards
...
# calculate background-normalized intensity for each file (mode measurement)
chroms.peak.norm = chroms.peak %>%
  group_by(File) %>%
    mutate(Background = modeest::mfv1(round(Intensity, digits = 1))) %>%
    mutate(Intensity_diff = Intensity - Background) %>%
  ungroup();

# calculate peak areas for each file
