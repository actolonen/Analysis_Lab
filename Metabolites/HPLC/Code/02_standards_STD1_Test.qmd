---
title: "HPLC: compute linear model to quantify compounds in standard solution (STD1)"
author: "Fermlab"
format: html
editor: visual
toc: TRUE
...
my.standards = standards %>%
  filter(grepl("STD1_", Sample)) %>%
  mutate(Concentration_mM = round(Concentration_mM, 2)) %>%
  select(Sample, Compound, Concentration_mM);

table.standards = kable(my.standards, caption = "Table 1: Metabolite concentrations in STD1 standards");
table.standards
...
Method 1: Calculate peak areas for a metabolite. Intensities are normalized by subtracting the mode intensity of the chromatogram.

```{r calculate peak areas for compound of interest (mode normalization)}

# input = data.frame chroms.peak = Minutes, Intensity, Sample, File
# output = data.frame areas.all = Area, Concentration_mM, Sample, File

calc_areas_mode = function(chroms.peak, my.compound)
{
...
Method 2: Calculate peak areas for a metabolite. Intensities are normalized by subtracting the mean of the values before and after the peak (peak boundaries). This method may be more accurate when the chromatogram baseline wanders.

```{r calculate peak areas for compound of interest (boundries normalization)}

# input = data.frame chroms.peak = Minutes, Intensity, Sample, File
# output = data.frame areas.all = Area, Concentration_mM, Sample, File

calc_areas_boundaries = function(chroms.peak, my.compound)
{
...
chroms.peak.norm = chroms.peak %>%
  group_by(File) %>%
    ...
    mutate(Intensity_diff = Intensity - Intensity_base) %>%
    mutate(Area = AUC(Minutes, Intensity_diff, method = 'trapezoid')) %>%
  ungroup();
...
if (peak.method == "mode")
{
  areas.concs = calc_areas_mode(chroms.peak, my.compound);
}
if (peak.method == "boundaries")
{
  areas.concs = calc_areas_boundaries(chroms.peak, my.compound);
}
